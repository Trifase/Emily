...
async def zoom_link(update: Update, context: CallbackContext):
    ...
    def get_access_token(acc_id, client_id, client_secret) -> str:
        ...
        data = {"grant_type": "account_credentials", "account_id": acc_id}
    ...
    def get_meeting_setting(token, meeting_uuid):
        ...
        response = requests.get(
            f"https://api.zoom.us/v2/meetings/{meeting_uuid}/recordings/settings", headers=headers
        ).json()
    ...
    def patch_default_permissions(token, meeting_uuid, oggi):
        ...
        response = requests.patch(
            f"https://api.zoom.us/v2/meetings/{meeting_uuid}/recordings/settings", headers=headers, json=new_settings
        )
