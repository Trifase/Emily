...
async def settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        for sett in all_settings:
            bottoni.append([InlineKeyboardButton(f"{sett['label']}: {user_settings[sett['chiave']]}", callback_data=f"sett:{sett['chiave']}")])
        ...
        msg = await update.message.reply_html(messaggio, reply_markup=reply_markup)
...
async def settings_change_actual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text.lower() == "annulla":
        ...
        await delete_all_messages(context, update.effective_chat.id, context.user_data['conversation_settings']['message_to_delete'])
        context.user_data.pop('conversation_settings', None)
    ...
    await update.effective_chat.send_message(f'Valore di <code>{chiave}</code> cambiato in <code>{update.message.text}</code>!')
