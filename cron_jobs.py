...
async def check_reminders(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for row in Reminders.select().where(Reminders.date_to_remind == datenow):
        if row:
            print(
                f'{get_now()} Trovato un reminder: "{row.message}" del {row.date_now}',
            )
            if not row.message:
...
async def plot_boiler_stats(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def get_room_history(
        access_token, home_id, room_id, date_end="last", graph_type="temperature", scale="1hour"
    ):
        headers = {
            "accept": "application/json",
            "Authorization": f"Bearer {access_token}",
...
async def do_global_backup(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(
            f"{get_now()} [AUTO] Backup globale eseguito: {archive_name} - File archiviati: {backupped} - File Skippati: {skipped} - File inviato su TG"
        )
    except Exception as e:
        print(
            f"{get_now()} [AUTO] Backup globale eseguito: {archive_name} - File archiviati: {backupped} - File Skippati: {skipped} - File NON inviato su TG\n{e}"
        )
