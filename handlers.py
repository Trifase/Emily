...
def generate_handlers_dict() -> dict:
    ...
    h[-1002] = [
        MessageHandler(
            ~filters.UpdateType.EDITED & ~filters.ChatType.CHANNEL & filters.TEXT, drop_update_from_banned_users
        )
    ]
    h[-20] = [
        MessageHandler(~filters.UpdateType.EDITED & ~filters.ChatType.CHANNEL & filters.TEXT, exit_from_banned_groups)
    ]
    h[-19] = [
        MessageHandler(~filters.UpdateType.EDITED & ~filters.ChatType.CHANNEL & filters.TEXT, nuova_chat_rilevata)
    ]
    h[-18] = [
        MessageHandler(
            ~filters.UpdateType.EDITED & ~filters.ChatType.CHANNEL & filters.TEXT, update_timestamps_asphalto
        )
    ]
    ...
    h[-12] = [
        CommandHandler(
            ["checktemp", "check_temp", "temp", "temperatura"],
            check_temp,
    ...
    h[-10] = [
        CommandHandler(
            ["screenshot", "ss"], screenshot, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF)
        )
    ]
    ...
    h[-6] = [
        CommandHandler(
            ["commandlist", "lista_comandi", "commands"],
            commandlist,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
    h[-5] = [
        CommandHandler(
            ["del", "cancella", "delete"], cancella, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF)
        )
    ]
    ...
    h[-2] = [
        CommandHandler(["ripeti", "echo"], echo, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF))
    ]
    h[-1] = [CommandHandler("info", tg_info, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF))]
    h[0] = [
        CommandHandler(
            ["getchat", "get_chat"], getchat, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF)
        )
    ]
    h[1] = [
        CommandHandler(
            ["chatlist", "listachat", "lista_chat"],
            lista_chat,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
    h[2] = [
        CommandHandler(
            ["listen_to", "listen"], listen_to, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF)
        )
    ]
    ...
    h[7] = [
        CommandHandler(
            ["addban", "ban", "add_ban"], add_ban, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF)
        )
    ]
    h[8] = [
        CommandHandler(
            ["delban", "del_ban", "unban", "sban"],
            del_ban,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
    ...
    h[10] = [
        CommandHandler(
            ["set_propic", "setpicture", "propic"],
            set_group_picture,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
    h[11] = [
        CommandHandler("backup", trigger_backup, filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF))
    ]
    h[12] = [
        CommandHandler(
            ["send_media", "send_custom_media", "send"],
            send_custom_media,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
    ...
    # banca.py
    h[16] = [CommandHandler("saldo", bot_get_saldo, filters=~filters.UpdateType.EDITED)]

    h[17] = [
        CommandHandler(
            ["movimenti", "transazioni"], bot_get_transazioni, filters=~filters.UpdateType.EDITED
        )
    ]
    ...
    h[19] = [
        CommandHandler(
            ["permasilenzia", "permasilenzio", "permataci", "shhhhhhhh", "permamute"],
            permasilenzia,
    ...
    h[24] = [
        CommandHandler(["cancellacompleanno", "delcompleanno"], compleanno_del, filters=~filters.UpdateType.EDITED)
    ]
    h[25] = [
        CommandHandler(
            "compleanni_check",
            compleanni_manual_check,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
    ...
    # diochan.py
    h[26] = [
        CommandHandler("quote", search_quote, filters=~filters.UpdateType.EDITED & filters.Chat(config.ID_DIOCHAN))
    ]
    h[27] = [
        CommandHandler("addquote", add_quote, filters=~filters.UpdateType.EDITED & filters.Chat(config.ID_DIOCHAN))
    ]
    ...
    h[-301] = [
        MessageHandler(~filters.UpdateType.EDITED & filters.TEXT & filters.User(username="@Tensor1987"), save_tensor)
    ]
    ...
    h[-401] = [
        MessageHandler(
            ~filters.UpdateType.EDITED & ~filters.ChatType.CHANNEL & filters.TEXT & filters.Chat(config.ID_LOTTO),
            maesta_primo,
        )
    ]
    ...
    h[52] = [
        CommandHandler(["trifase", "randomtrif", "randomtrifase"], random_trifase, filters=~filters.UpdateType.EDITED)
    ]
    ...
    h[-21] = [
        ConversationHandler(
            ...
            states={
                ...
                -2: [TypeHandler(Update, end_conversation)],
            },
            fallbacks=[],
    ...
    h[65] = [
        MessageHandler(
            ...
            & filters.Regex(re.compile(r"[Aa]lexa (?:[Pp]lay|[Rr]iproduci) (.+)", re.IGNORECASE)),
            alexa,
        )
    ]
    ...
    # pyrog.py
    h[77] = [
        CommandHandler(["karma", "reactionlist", "reactkarma"], reaction_karma, filters=~filters.UpdateType.EDITED)
    ]
    ...
    h[85] = [
        CommandHandler(
            ["reminderlist", "remindlist", "listareminder", "reminder_list"],
            reminders_list,
            filters=~filters.UpdateType.EDITED,
        )
    ]
    h[86] = [
        CommandHandler(
            ["reminder_delete", "remindelete", "deletereminder"], remindelete, filters=~filters.UpdateType.EDITED
        )
    ]
    ...
    h[98] = [
        MessageHandler(
            ~filters.UpdateType.EDITED & filters.Regex(re.compile(regex_reddit, re.IGNORECASE)), parse_reddit_link
        )
    ]
    h[99] = [
        MessageHandler(
            ~filters.UpdateType.EDITED & filters.Regex(re.compile(regex_twitch_clip, re.IGNORECASE)), twitch_clips
        )
    ]
    ...
    h[104] = [
        CommandHandler(
            ["status_luci", "statusluci", "luci", "luci_status"], luci_status, filters=~filters.UpdateType.EDITED
        )
    ]
    ...
    h[108] = [
        CommandHandler(["riscaldamento", "boiler", "caldaia"], riscaldamento_stats, filters=~filters.UpdateType.EDITED)
    ]
    ...
    h[111] = [
        MessageHandler(~filters.UpdateType.EDITED & filters.Regex(re.compile(regex_spotify, re.IGNORECASE)), spoty)
    ]
    ...
    h[114] = [
        CommandHandler(["tarotschema", "reversetarots", "schema"], tarotschema, filters=~filters.UpdateType.EDITED)
    ]
    ...
    h[117] = [
        CommandHandler(
            ["torrents", "torrentlist", "listatorrent", "lista_torrent"],
            lista_torrent,
            filters=~filters.UpdateType.EDITED & filters.User(config.ID_TRIF),
        )
    ]
