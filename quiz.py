...
async def make_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    difficuly_time = {"easy": 15, "medium": 30, "hard": 60}
...
async def ricevi_risposta_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if risposta == context.bot_data["trivia"][poll_id][1]:
        ...
        print(
            f'{get_now()} {await get_display_name(update.effective_user)} risponde a {poll_id} ({difficulty}) correttamente! Ora ha {context.user_data["_points_new"]} punti.'
        )
        ...
        print(
            f'{get_now()} {await get_display_name(update.effective_user)} risponde a {poll_id} ({difficulty}) ma sbaglia! Ha sbagliato {context.user_data["trivia_wrongs"]} volte.'
        )
